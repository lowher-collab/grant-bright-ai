  ...
  return (
    <section className="py-24 bg-slate-900 text-white text-center">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl md:text-5xl font-bold mb-8">Ready to recover your unclaimed capital?</h2>
        <Button onClick={handleClick} disabled={isLoading} className="bg-green-500 hover:bg-green-600 text-white font-bold px-10 py-8 text-xl rounded-2xl shadow-xl shadow-green-500/20 transition-all">
          {isLoading ? <Loader2 className="animate-spin" /> : <span className="flex items-center gap-2">Check Eligibility Now <ArrowRight /></span>}
        </Button>
      </div>
    </section>
  );
